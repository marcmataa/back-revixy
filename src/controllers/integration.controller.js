...
const handleShopifyCallback = async (req, res) => {
  try {
    ...
    const token = await shopifyService.exchangeCodeForToken(shop, code, state, req.user.id);
    const defaults = { defaultMarginPercent: 30, defaultShippingCost: 0 };
    const store = await Store.findOneAndUpdate(
      { shopifyDomain: String(shop).toLowerCase() },
      { owner: req.user.id, shopifyDomain: String(shop).toLowerCase(), accessToken: token, timezone: "UTC", settings: defaults, status: "ACTIVE" },
      { new: true, upsert: true, runValidators: true, setDefaultsOnInsert: true }
    );
    await ActionLogs.create({ storeId: store._id, userId: req.user.id, type: "AUTH_EVENT", status: "SUCCESS", message: "Shopify connected successfully", metadata: { provider: "shopify", shopifyDomain: `${String(shop).slice(0, 3)}***` } });
